...
def build_type1gfs_15r_relaxed2():
    nodes: List[Dict] = []
    ...
    spec = dict(
        ...
        tails=tails,
        allow_weak=True,
        sum1=False,            # relaxed sum1
        monotone_guard=False,  # relaxed guard
        collisions=True,       # keep collisions so 0s can arise
    )
    return spec, rounds

def main():
    spec, rounds = build_type1gfs_15r_relaxed2()
    ...
    for ri in range(0, 16):
        ...
        if ri == 12:
            print("[DEBUG] Round 12 raw BV:", raws)
