...
"""
td_then_smt_Type1GFS_from_r5.py
...
流程：
1) TD 域（td_trunc.py）从 Round 0 输入 (s, a, 0, 0) 开始，跑到 Round 5 并打印 0..5 轮的 TD 符号；
2) SMT 域（smt_model_distinguish.py）把 Round 5 的 4 根线“固定”为：
      u0^5 = 0          → #b00000 (0)
      u1^5 = x          → #b00100 (4)
      u2^5 = δ ⊕ R(x)   → #b01001 (9)
      u3^5 = R(δ)       → #b00010 (2)
   然后继续跑 10 轮（Round 6..15），打印每一轮 SMT 符号/⊥。
"""
...
def build_type1gfs_10r_from_r5():
    """
    Round-5 视为“第 0 轮输入”，构建 10 轮到“Round-15”尾部。
    """
    nodes: List[Dict] = []
    # build_from_graph exposes graph inputs as u0_0, ..., u0_3.  They represent
    # Round 5 here even though the internal names use the model's round-zero
    # convention.
    rounds = [ { f"u{k}": f"u0_{k}" for k in range(4) } ]
    ...
    # Tails correspond to overall Round-15
...
def run_smt_from_round5_and_print():
    ...
    s.assertFormula(s.mkTerm(Kind.EQUAL, env["inputs"][2], mkbv(9 )))  # u2^5 = δ ⊕ R(x) = 1|8=9
