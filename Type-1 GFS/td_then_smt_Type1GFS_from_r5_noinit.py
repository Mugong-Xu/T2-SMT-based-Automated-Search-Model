...
def build_type1gfs_10r_from_r5_relaxed():
    """
    用 Round‑5 作为“输入”，并以更宽松的 spec 构图，减少输入域约束的影响：
    ...
    spec = dict(
        ...
        tails=tails,
        allow_weak=True,
        sum1=False,            # 关闭 sum1 以避免输入域守恒过强
        monotone_guard=False,  # 关闭单调守卫，减少 ⊥ 传播
        collisions=True,       # 保留碰撞系统（允许 0s 的出现）
    )
    return spec, rounds
...
def run_smt_from_round5_and_print():
    ...
    # 固定 Round‑5 的 SMT 种子：u0^5=0, u1^5=x, u2^5=δ⊕R(x), u3^5=R(δ)
    ...
    s.assertFormula(s.mkTerm(Kind.EQUAL, env["inputs"][2], mkbv(9 )))  # δ(1) | R(x)(8) = 9
