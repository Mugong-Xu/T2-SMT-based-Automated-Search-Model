...
def build_type1gfs_15r():
    ...
    # collect all R-node names and their input refs to add custom guards
    r_nodes = []
    for i in range(15):
        cur = rounds[-1]
        R3 = f"R3_{i}"
...
def add_R_singleton_guards(s, env, r_nodes):
    """For every R node, assert: if input has >=2 hot bits among {δ,Rδ,x,Rx,0s}, then bot(R)=True."""
    mkbv = lambda x: s.mkBitVector(5, x)
    zero = mkbv(0)
    masks = [1,2,4,8,16]  # δ, Rδ, x, Rx, 0s
    # precompute pair masks
...
def main():
    ...
    # Enforce singleton-input semantics for R
    add_R_singleton_guards(s, env, r_nodes)
    ...
    # Print Round 6 and 7 for verification, then all rounds
    for ri in [6,7]:
