...
"""
td2smt_GFS4F_from_r4_sliced_relaxed.py

目的：当从 TD Round-4 切到 SMT 时，如果严格约束导致 UNSAT，
使用一个“放松约束”的 SMT 后缀（关闭 sum1/monotone_guard/collisions，保留 allow_weak）
以便验证可满足性与符号传播是否合理。
"""
...
def build_smt_suffix_relaxed():
    nodes = []
    rounds = [ { f"u{k}": f"u0_{k}" for k in range(8) } ]  # standard inputs
    for i in range(6):
        cur = rounds[-1]
    ...
    spec = dict(
        ...
        allow_weak=True,
        sum1=False,             # relaxed
        monotone_guard=False,   # relaxed
        collisions=False,       # relaxed
    )
    return spec, rounds
...
def main():
    ...
    # SMT后缀（放松约束）
    spec, rounds = build_smt_suffix_relaxed()
    ...
    for k in range(8):
        mk = masks[k]
        if mk is not None:
            s.assertFormula(s.mkTerm(Kind.EQUAL, env["inputs"][k], mkbv(mk)))
