# -*- coding: utf-8 -*-
# === smt_model_distinguish_new_IV ===
# XOR δ-bit uses BVXOR (δ⊕δ=0) in add_XOR_node_no_collision and in the
# non-collision branch of add_XOR_node. x-bit was already BVXOR.
# Other bits (R(δ), R(x), 0s) keep base semantics.
...
def add_XOR_node_no_collision(s, A, B, name: str, Abot=None, Bbot=None):
    ...
    s.assertFormula(Eq(s, w4, s.mkTerm(Kind.BITVECTOR_XOR, u4, v4)))  # δ 位
...
def add_XOR_node(s, A, B, name: str, Abot=None, Bbot=None):
    ...
    s.assertFormula(Imp(s, not_x,
                        And2(s,
                             ...
                             Eq(s, w4, s.mkTerm(Kind.BITVECTOR_XOR, u4, v4)),
                             )
                        ))
...
def build_from_graph(spec):
    ...
    if True and R_flags:  # force-enable Eq.(6)/(7) even if spec['sum1'] is False
        # at most one s_i
