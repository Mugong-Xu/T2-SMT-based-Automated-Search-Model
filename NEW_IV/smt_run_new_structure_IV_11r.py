...
def build_newstructIV_spec(depth: int, allow_weak: bool, sum1: bool, monotone_guard: bool, collisions: bool):
    ...
    # Tails are the last round's wires (round 'depth')
...
def main():
    ...
    default_model = os.path.join(here, "smt_model_distinguish_new_IV.py")  # δ⊕δ=0, x⊕x=0 base

    ap = argparse.ArgumentParser()
    ap.add_argument("--model", default=default_model)
    ap.add_argument("--depth", type=int, default=11, help="number of forward rounds (prints 1..depth)")
    ...
    spec, rounds = build_newstructIV_spec(
        depth=args.depth,
