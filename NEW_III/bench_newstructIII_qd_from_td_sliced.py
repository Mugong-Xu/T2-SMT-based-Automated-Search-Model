...
def td_to_smt_mask(td, v) -> Optional[int]:
    ...
    if v.sym == Sym.A:
        ...
        return (0b10001 if a_flag==1 else 0b00001)  # δ or 0s⊕δ（按你的 td 设计可再细化）
    if v.sym == Sym.STAR:    return None
    return None

# ===== Build only the suffix (anchor..13) in SMT =====

def build_spec_suffix_from_anchor(anchor_round: int):
    """Build only the suffix from 'anchor_round' to MAX_ROUNDS, using standard inputs u0_0..u0_3
    as the values at round = anchor_round.
    """
    steps = MAX_ROUNDS - anchor_round  # number of forward steps to reach MAX_ROUNDS
    nodes = []
    rounds = []
    # Round 'anchor_round' wires are just the standard inputs (names u0_0..u0_3 in outmap).
    rounds.append({"u0": "u0_0", "u1": "u0_1", "u2": "u0_2", "u3": "u0_3"})
    for i in range(steps):
        cur = rounds[-1]
        r_name = f"R_suf_{anchor_round+i}"
        x_name = f"X_suf_{anchor_round+i}"
...
# ===== Structural decrypt-prune (unchanged) =====
...
def main():
    # TD step to get anchor and masks
    td = load_td_trunc()
    ...
    # Build *suffix-only* spec once
    mod = load_user_model(MODEL_PATH)
    spec, rounds = build_spec_suffix_from_anchor(r_anchor)

    # Warmup
    for _ in range(3):
        s, env = mod.build_from_graph(spec)
        mkbv = lambda x: s.mkBitVector(5, x)
        # Assert the 4 anchor inputs to the given masks
        for i in range(4):
            s.assertFormula(s.mkTerm(Kind.EQUAL, env["inputs"][i], mkbv(anchor_masks[i])))
        _ = s.checkSat()
        gc.collect()
    ...
    for _ in range(REPEAT):
        ...
        # Survivor always interpreted at absolute round MAX_ROUNDS;
        # here we only built rounds from anchor..MAX_ROUNDS, but the *names* inside 'rounds'
        # still use rel semantics; the last item corresponds to MAX_ROUNDS
        survivor_round, survivor_nm = decrypt_prune_survivor()
        # Map reference in suffix builder
        ref = rounds[-1][survivor_nm] if survivor_nm is not None else None
        if ref is not None:
