...
使用 SMT 计算 NEW_IV_ENC 前 5 轮的符号传播。
输出 Round 0 ~ Round 5 每一轮四个分支的符号。
QD 判定仍然基于 Round-5 最后一轮四个分支中，任一分支同时包含 x-family 与 δ-family。
不执行剪枝，不统计平均时间。
"""
...
def run_once(mod):
    ...
    if str(res).lower() != "sat":
        return False, [[f"u{i}^{r}: ⊥" for i in range(4)] for r in range(ROUNDS_N + 1)]

    has_qd = False
    all_round_lines = []
    for r in range(ROUNDS_N + 1):
        round_lines = []
        for nm in ["u0", "u1", "u2", "u3"]:
            bits, bot = fetch_value_bits(s, env, rounds[r][nm])
            if bot:
                line = f"{nm}^{r}: ⊥"
            else:
                sym = bits5_to_symbol(bits)
                line = f"{nm}^{r}: {sym}"
                if r == ROUNDS_N and has_quantum_distinguisher(bits):
                    has_qd = True
            round_lines.append(line)
        all_round_lines.append(round_lines)
    return has_qd, all_round_lines


def main():
    mod = load_model(MODEL_DEFAULT)
    qd, all_round_lines = run_once(mod)


    for r, round_lines in enumerate(all_round_lines):
        print(f"Round {r}:")
